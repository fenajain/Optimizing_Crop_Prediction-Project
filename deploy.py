...
def crop_prediction(ip_data):
    ...
    num_data = reshaped_data.astype(
        np.float64)  # The value received from above is in string form so to convert that into string we did this
    return num_data
...
try:
    if st.button("Predict & Show Analysis"):
        ...
        res = np.array(result)
        st.write("<span style = 'font-size : 24px'> The output is <u><b>{}</b></u></span>".format(res[0].capitalize()),
                 unsafe_allow_html=True)

        crop_name = res[0]
        seasonal_crop = suggest_season(crop_name, df)
        st.write("<span style = 'font-size : 24px'> Your crop {} is <u><b>{}</b></u> crop</span>".format(res[0],
                                                                                                         seasonal_crop.capitalize()),
                 unsafe_allow_html=True)
        ...
        st.markdown(
            "<h1 style='text-align: left; font-size: 30px;'>Different Quantity of Chemical Content in {} crop</h1>".format(
                res[0].capitalize()), unsafe_allow_html=True)
    ...
    # if st.button("Data_Analytics"):
        ...
        st.markdown("<h1 style='text-align: left; font-size: 30px;'>Overall Data Visualization </h1>",
                    unsafe_allow_html=True)
    ...
    st.markdown("<h1 style='text-align: left; font-size: 24px;'>Some Interesting Facts to known about crops</h1>",
                unsafe_allow_html=True)
    if st.button("Facts"):
